...
private:
  ...
  bool openSocket(const std::string &hostname, const std::string &port, int options) {
    ...
    if (err != 0) {
        #ifdef _WIN32 || _WIN64
        //WCHAR to std::string
        std::wstring w_err = gai_strerror(err);
        int len;
        int slength = (int)w_err.length() + 1;
        len = WideCharToMultiByte(CP_ACP, 0, w_err.c_str(), slength, 0, 0, 0, 0);
        char* buf = new char[len];
        WideCharToMultiByte(CP_ACP, 0, w_err.c_str(), slength, buf, len, 0, 0);
        std::string s_err(buf);
        delete[] buf;
        setErr((s_err));
        //end
        #else
